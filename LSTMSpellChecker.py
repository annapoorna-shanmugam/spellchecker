"""
LSTM-based Neural Spell Checker Module

This module implements a spell checker using a bidirectional LSTM (Long Short-Term Memory)
neural network with an encoder-decoder architecture. It provides domain-specific spell
checking capabilities for general, medical, and legal text.

The spell checker combines neural network predictions with Levenshtein distance-based
suggestions to provide accurate spelling corrections while considering domain context.

Key Features:
    - Encoder-decoder LSTM architecture for context-aware spell checking
    - Domain-specific vocabulary handling (medical, legal, general)
    - Synthetic error generation for training
    - Confidence scoring based on multiple factors

Dependencies:
    - tensorflow: For neural network implementation
    - numpy: For numerical computations
    - random: For synthetic error generation

Author: Ann Shanmugam
Date: June 2025
"""
...
class LSTMSpellChecker:
    """
    A neural spell checker class using LSTM networks for context-aware spell checking.
    
    This class implements an encoder-decoder architecture using bidirectional LSTM
    networks to provide context-aware spelling suggestions. It supports domain-specific
    spell checking and includes synthetic error generation for training.

    Attributes:
        clean_sentences (list): List of correct sentences for training
        clean_words (list): List of domain-specific vocabulary words
        noisy_sentences (list): List of sentences with synthetic errors
        labels (list): Labels indicating positions of errors in noisy sentences
        tokenizer: Keras tokenizer for text processing
        model: Keras model for spell checking
        max_len (int): Maximum sentence length for padding
    """
    
    def __init__(self):
        """Initialize the LSTM spell checker with empty attributes."""
    ...
    def inject_error(self, sentence):
        """
        Generate synthetic spelling errors in a sentence.

        Args:
            sentence (str): Input sentence to modify

        Returns:
            tuple: (wrong_sentence, wrong_indices)
                - wrong_sentence: The sentence with injected errors
                - wrong_indices: List of indices where errors were injected

        The function applies various types of realistic spelling errors:
        - Character swaps
        - Character insertions
        - Character deletions
        - Character replacements with similar-looking characters
        """
    ...
    def prepare_model(self):
        """
        Prepare and train the LSTM model for spell checking.

        This method handles:
        1. Generation of training data with synthetic errors
        2. Text tokenization and sequence padding
        3. Building the encoder-decoder LSTM model
        ...
        Raises:
            Exception: If there's an error during model preparation or training
        """
        try:
    ...
    def correct_sentence(self, test_sentence, domain='general'):
        """
        Check and correct spelling in a sentence.

        Args:
            test_sentence (str): The input sentence to check
            domain (str): Domain for specialized spell checking ('general', 'medical', or 'legal')

        Returns:
            list: List of dictionaries containing error information:
                - word: The potentially misspelled word
                - position: Word position in the sentence
                - suggestions: List of (suggestion, confidence) pairs
                - type: Error type ('general' or 'domain')
                - confidence: Overall confidence score for the suggestion

        The method combines neural network predictions with Levenshtein distance-based
        suggestions and applies domain-specific knowledge for better accuracy. The
        confidence score is calculated using multiple factors:
        1. String similarity (Levenshtein distance)
        2. Model prediction probability
        3. Domain-specific knowledge
        """
        if domain == 'general':
            with open('dataset/general_sentences.txt', 'r') as f:
        ...
        if domain == 'medical':
            with open('dataset/medical_sentences.txt', 'r') as f:
                self.clean_sentences = [line.strip() for line in f if line.strip()]
            self.clean_words = [
                "diagnosis", "infection", "treatment", "medication", "antibiotics",
                "inflammation", "swelling", "biopsy", "symptoms", "therapy",
        ...
        if domain == 'legal':
            with open('dataset/legal_sentences.txt', 'r') as f:
                self.clean_sentences = [line.strip() for line in f if line.strip()]
            self.clean_words = [
